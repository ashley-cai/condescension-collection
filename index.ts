...
let wordCheckerInt: number[] =[] 
let wordCheckerWord: string[] =[]
let wordCheckerLetter: string[] =[]

let userTypedLetter: string[] = []
...
function mixKeyBindings(count: number){
    ...
    for (let i = 0; i < countMixedLetters; i++) {
        ...
        for (let inputElement of inputElements){
            inputElement.addEventListener('keydown', function(event) { if (event.key === letter.toLowerCase() || event.key === letter.toUpperCase()) { 
...
function getRandomLetter(not: string) {
...
//VERBALLY BERATING
function berate(wordMistake: string) {
    let berateContainer = document.getElementById("berate-container");
    let coords = getRandomCoordinatesOutsideDiv("game-container")
    berateContainer?.innerHTML += ("<div class='disappearingText berate' onmouseover='vanish(this)' style='transform: translate("+ coords.x +"px,"+ coords.y +"px)'>"+ wordMistake+"</div>")
    setTimeout(function(){
